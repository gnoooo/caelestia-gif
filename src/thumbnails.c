 ...
 */
static int thumbnails_needs_regeneration(const char *gifpath, const char *thumbpath) {
    if (!gifpath || !thumbpath) {
        return 0;
    }
...
void thumbnails_generate(const Config *cfg, const char *gif_dir, const char *thumb_dir, int clean_thumbs) {
    if (!cfg || !gif_dir || !thumb_dir) {
        fprintf(stderr, "Error: NULL parameter passed to thumbnails_generate\n");
        return;
    }
    ...
    if (ensure_dir(thumb_cache_dir) != 0 || ensure_dir(thumb_dir) != 0) {
    ...
    while ((ent = readdir(d))) {
        ...
        const char *thumbparts[] = {thumb_dir, "/", name};
    ...
    while ((ent = readdir(td))) {
        ...
        if (access(srcpath, F_OK) != 0) {
            const char *tpath_parts[] = {thumb_dir, "/", ent->d_name};
